"""Bitcoin OTC/Alpha dataset loader and processor.

Loads and preprocesses temporal graph data from Bitcoin transaction networks.
"""
...
class bitcoin_dataset:
    """Bitcoin dataset with temporal edge information."""

    def __init__(self, args):
        """Initialize and load the Bitcoin dataset.
        ...
        assert args.task in ["link_pred", "edge_cls"], (
            "bitcoin only implements link_pred or edge_cls"
        )
        self.ecols = u.Namespace({"FromNodeId": 0, "ToNodeId": 1, "Weight": 2, "TimeStep": 3})
        args.bitcoin_args = u.Namespace(args.bitcoinotc_args)
        ...
        # separate classes
        sp_indices = edges[
            :, [self.ecols.FromNodeId, self.ecols.ToNodeId, self.ecols.TimeStep]
        ].t()
        ...
        neg_sp_edges = torch.sparse.LongTensor(
            neg_sp_indices, neg_sp_values, torch.Size([num_nodes, num_nodes, self.max_time + 1])
        ).coalesce()
        ...
        pos_sp_edges = torch.sparse.LongTensor(
            pos_sp_indices, pos_sp_values, torch.Size([num_nodes, num_nodes, self.max_time + 1])
        ).coalesce()
    ...
    def load_edges(self, bitcoin_args):
        ...
        Args:
            bitcoin_args: Configuration with folder and edges_file paths.

        Returns:
            Tensor of edges with (source, target, weight, timestamp).
        """
