"""Utilities to parse and analyze experiment log files."""
...
if (
    ...
    target_measure = "f1"  # map f1 p r loss avg_p avg_r avg_f1
else:
    target_measure = "map"  # map f1 p r loss avg_p avg_r avg_f1
...
with open(filename) as f:
    ...
    for line in f:
        ...
        if "Best epoch: " in line:
            best_epoch_from_log = int(line.split("Best epoch: ")[1])
        if params_line:
            line = line.replace("INFO:root:", "").replace("\n", "")
        if params_line:
        ...
        if "TRAIN epoch" in line or "VALID epoch" in line or "TEST epoch" in line:
            ...
            errors[set][epoch] = v
            if target_measure == "errors" and v < best_measure[set]:
                best_measure[set] = v
                best_epoch[set] = epoch
        elif "mean losses" in line:
            v = float(line.split("(")[1].split(")")[0].split(",")[0])
            losses[set][epoch] = v
            if target_measure == "loss" and v > best_measure[set]:
                best_measure[set] = v
                best_epoch[set] = epoch
        elif "mean MAP" in line:
            v = float(line.split("mean MAP ")[1].split(" ")[0])
            MAPs[set][epoch] = v
            if target_measure == "map" and v > best_measure[set]:
                best_measure[set] = v
                best_epoch[set] = epoch
            if set == "TEST":
                last_test_ep["MAP"] = v
        elif "measures macroavg" in line:
...
try:
    ...
    if print_params:
        print("str_params:\n", str_params)
    if best_epoch_from_log >= 0:
        best_ep = best_epoch_from_log
        print("Using best epoch from log: %d" % best_ep)
    elif best_epoch["VALID"] >= 0:
    ...
    try:
        ...
        print(
            "Values at best Valid Epoch (%d) macro-AVG: TEST Precision %0.4f - Recall %0.4f - F1 %0.4f"
            % (
                best_ep,
                prec["TEST"][best_ep],
                rec["TEST"][best_ep],
        ...
        res_map["AVG-recall"] = last_test_ep["AVG-recall"]
    ...
    print(f"Some error occurred in {filename} - Epochs read: {epoch} - {exc}")
    exit(0)
