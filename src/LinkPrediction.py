...
class LinkPrediction:
    ...
    def get_sample(self, idx, test, **kwargs):
        ...
        if len(hist_adj_list) == 0:
            ...
            # Use features from previous snapshot (or current if idx=0)
            feat_time = max(0, idx - 1)
            node_feats = self._get_node_features_for_active_nodes(
                feat_time, active_node_indices, max_allowed_timestep=None
            )
            node_feats = node_feats.detach()
        ...
        # IMPORTANT: Use time_window=1 to get ONLY edges at this specific snapshot,
        # avoiding temporal leakage from historical edges already seen in training
        label_adj = tu.get_sp_adj(
            edges=self.dataset.edges,
            time=idx,
            time_window=1,  # Only edges at snapshot idx, not cumulative
        )
        ...
        # For cold start (num_hist_steps=0), existing_nodes is empty
        # Use all active nodes for smart negative sampling (already in compacted space)
        if len(existing_nodes) == 0:
            # Create a tensor with all compacted node IDs (0 to num_active_nodes-1)
            existing_nodes = torch.arange(num_active_nodes, dtype=torch.long)
        else:
            existing_nodes = torch.cat(existing_nodes)

        # Check if label_adj has any edges after remapping
        # If not, we can't generate meaningful negative samples from the positive distribution
        if label_adj["idx"].size(0) == 0:
