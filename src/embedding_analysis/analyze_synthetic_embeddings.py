"""Analyze synthetic BERT embeddings distribution and variance."""
...
def main():
    """Analyze synthetic embeddings distribution and save stats and plots."""
    ...
    print("=" * 60)
    print("SYNTHETIC BERT EMBEDDINGS ANALYSIS")
    print("=" * 60)
    ...
    cosine_dist = {}
    for low, high in [(0.0, 0.5), (0.5, 0.7), (0.7, 0.8), (0.8, 0.9), (0.9, 0.95), (0.95, 1.0)]:
    ...
    with open(output_dir / "synthetic_embeddings_stats.json", "w") as f:
        json.dump(stats, f, indent=2)
    print(f"\nSaved stats to {output_dir / 'synthetic_embeddings_stats.json'}")
    ...
    plt.ylabel("Count")
    plt.title("Synthetic Embeddings: Cosine Similarity Distribution")
    ...
    plt.ylabel("Count")
    plt.title("Synthetic Embeddings: Euclidean Distance Distribution")
    plt.axvline(
        euclidean_dist.mean(),
        color="red",
        linestyle="--",
    ...
    plt.tight_layout()
    plt.savefig(output_dir / "synthetic_embeddings_distribution.png", dpi=150)
    plt.close()
    print(f"Saved plot to {output_dir / 'synthetic_embeddings_distribution.png'}")

    plt.figure(figsize=(10, 4))
    plt.bar(range(768), var_per_dim)
    plt.xlabel("Dimension")
    plt.ylabel("Variance")
    plt.title("Variance per Dimension - Synthetic Embeddings")
    plt.tight_layout()
    plt.savefig(output_dir / "synthetic_variance_per_dim.png", dpi=150)
    plt.close()
    print(f"Saved plot to {output_dir / 'synthetic_variance_per_dim.png'}")
