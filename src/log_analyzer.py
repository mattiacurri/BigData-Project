"""Log analyzer for parsing training results from experiment logs.

Extracts metrics and hyperparameters from training logs for analysis and visualization.
"""
...
with open(filename) as f:
    ...
    for line in f:
        ...
        if "TRAIN epoch" in line or "VALID epoch" in line or "TEST epoch" in line:
            ...
            if (
                target_measure == "avg_p"
                or target_measure == "avg_r"
                or target_measure == "avg_f1"
            ):
            ...
            if (
                target_measure == "p" or target_measure == "r" or target_measure == "f1"
            ) and cl == cl_to_plot_id:
                if target_measure == "p":
            ...
            if (
                target_measure == "p@k" or target_measure == "r@k" or target_measure == "f1@k"
            ) and cl == cl_to_plot_id:
...
try:
    ...
    if best_epoch["VALID"] >= 0:
        ...
        print(
            "Highest %s values among all epochs: TRAIN  %0.4f\tVALID  %0.4f\tTEST %0.4f"
            % (target_measure, best_measure["TRAIN"], best_measure["VALID"], best_measure["TEST"])
        )
    else:
        best_ep = best_epoch["TEST"]
        print(
            "Highest %s values among all epochs:\tTRAIN F1 %0.4f\tTEST %0.4f"
            % (target_measure, best_measure["TRAIN"], best_measure["TEST"])
        )

    use_latest_ep = True
    try:
        print(
            "Values at best Valid Epoch (%d) for target class: TEST Precision %0.4f - Recall %0.4f - F1 %0.4f"
            % (
                best_ep,
                prec_cl["TEST"][cl_to_plot_id][best_ep],
                rec_cl["TEST"][cl_to_plot_id][best_ep],
                f1_cl["TEST"][cl_to_plot_id][best_ep],
            )
        )
        print(
            "Values at best Valid Epoch (%d) micro-AVG: TEST Precision %0.4f - Recall %0.4f - F1 %0.4f"
            % (best_ep, prec["TEST"][best_ep], rec["TEST"][best_ep], f1["TEST"][best_ep])
        )
    ...
    try:
        print(
            "Values at best Valid Epoch (%d) for target class@%d: TEST Precision %0.4f - Recall %0.4f - F1 %0.4f"
            % (
                best_ep,
                eval_k,
                prec_at_k_cl["TEST"][cl_to_plot_id][best_ep],
                rec_at_k_cl["TEST"][cl_to_plot_id][best_ep],
        ...
        print(
            ...
            % (
                best_ep,
                eval_k,
                prec_at_k["TEST"][best_ep],
                rec_at_k["TEST"][best_ep],
    ...
    try:
        print(
            "Values at best Valid Epoch (%d) MAP: TRAIN  %0.8f - VALID %0.8f - TEST %0.8f"
            % (best_ep, MAPs["TRAIN"][best_ep], MAPs["VALID"][best_ep], MAPs["TEST"][best_ep])
        )
    ...
    try:
        print(
            "Values at best Valid Epoch (%d) MRR: TRAIN  %0.8f - VALID %0.8f - TEST %0.8f"
            % (best_ep, MRRs["TRAIN"][best_ep], MRRs["VALID"][best_ep], MRRs["TEST"][best_ep])
        )
